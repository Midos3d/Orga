...
function refinePromptResult(promptResult,end) {
  ...
  if (indexOfNull < 0) {
    ...
    return (end)? promptResult.slice(0, indexOfNull + 1):promptResult.slice(0, indexOfNull );
  }
}
...
const result = await model.generateContent(data.replace("<user prompt>",pro));
      ...
      if(data["indx"]<0){ 
       ...
       const S = JSON.stringify(Data[indx]);
        ...
        if(Data[data["indx"]]["duration"]==null){
          ...
          Array.prototype.splice.apply(Data, [data["indx"], 0].concat(refinedData));
          if(indx==data["indx"]){
            waiting=true;
          indx--;
          }
          ...
          Array.prototype.splice.apply(Data, [data["indx"]+1, 0].concat(refinedData));
        }
        if(waiting){
          indx++;
          const S = JSON.stringify(Data[indx]);
